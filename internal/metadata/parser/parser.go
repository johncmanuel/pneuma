...
// ParseFrom is like ParseFile but works from an already-opened, seeked-to-zero
// *os.File. The caller retains ownership of f (it is not closed). This allows
// the scanner to open each file once, stream it through a hasher, seek back,
// then call ParseFrom — cutting the per-file read count from two passes to one.
func (p *Parser) ParseFrom(_ context.Context, path string, f *os.File) (*models.Track, error) {
	info, err := f.Stat()
	if err != nil {
		return nil, err
	}
	...
	if err == nil {
		...
		t.TrackNumber = tn
		t.DiscNumber = dn
		if t.AlbumArtist == "" && m.Artist() != "" {
			t.AlbumArtist = m.Artist()
		}
	}

	// ffprobe enrichment re-uses the path; no extra file open.
	if p.ffprobePath != "" {
		_ = p.probe(context.Background(), path, t)
	}
	// Pure-Go duration fallback: seek back to start so we can re-read the header.
	if t.DurationMS == 0 {
		if _, seekErr := f.Seek(0, io.SeekStart); seekErr == nil {
			_ = parseDurationFallbackFrom(f, path, t)
		}
	}
...
// parseDurationFallbackFrom tries to read duration from an already-opened
// io.ReadSeeker. Currently handles FLAC; other formats fall through silently.
func parseDurationFallbackFrom(r io.ReadSeeker, path string, t *models.Track) error {
	switch strings.ToLower(filepath.Ext(path)) {
	case ".flac":
		return parseFLACDurationFrom(r, t)
	}
	return nil
}

// parseFLACDurationFrom reads the FLAC STREAMINFO block from an io.ReadSeeker.
func parseFLACDurationFrom(r io.ReadSeeker, t *models.Track) error {
	magic := make([]byte, 4)
