...
async function injectTrackIntoQueue(newTrack: LocalTrack) {
  ...
  if (s.track.album_name !== newTrack.album ||
      s.track.album_artist !== newTrack.album_artist) return
  ...
  try {
    albumTracks = await fetchLocalAlbumTracks(newTrack.album, newTrack.album_artist)
  } catch {
    return
  }
  ...
  playerState.update(cur => {
    ...
    return { ...cur, queue: newQueue, baseQueue: newBase, queueIndex: newQueueIndex }
  })
}
...
export async function initLocalLibrary(): Promise<void> {
  ...
  if (folders.length > 0) {
    ...
    for (const dir of folders) {
      WatchLocalFolder(dir).catch((e) => console.warn("WatchLocalFolder failed:", dir, e))
    }
  }
  ...
  EventsOn("local:track:removed", (data: { path?: string; paths?: string[] }) => {
    if (data.paths) {
...
export async function removeLocalFolder(dir: string) {
  ...
  UnwatchLocalFolder(dir).catch((e) => console.warn("UnwatchLocalFolder failed:", dir, e))
